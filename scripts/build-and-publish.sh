...
rm -f "$PROJECT_DIR/public/mamlaka-tv-app.apk"
echo ""
...
cp "$PROJECT_DIR/android/app/build/outputs/apk/release/app-release.apk" \
   "$PROJECT_DIR/public/mamlaka-tv-app.apk"
echo "  ✅ APK copied to public/mamlaka-tv-app.apk"

# Get file size
APK_SIZE=$(stat --format=%s "$PROJECT_DIR/public/mamlaka-tv-app.apk" 2>/dev/null || \
           stat -f%z "$PROJECT_DIR/public/mamlaka-tv-app.apk" 2>/dev/null || echo "0")
...
git add android/app/build.gradle public/mamlaka-tv-app.apk 2>/dev/null || true
...
echo "  APK: public/mamlaka-tv-app.apk"
